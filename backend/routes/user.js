...
// Route to handle resume upload and return file path
...
router.put("/application/:id", async (req, res) => {
  try {
    ...
    if (result[0] === 0) {
      // Sequelize returns an array, [0] means no rows were updated
      return res.status(404).json({ message: "Application not found" });
    }
...
router.put("/applications/soft-delete/:id", async (req, res) => {
  try {
    const result = await Application.update(
      { isDeleted: req.body.isDeleted, dateDeleted: req.body.dateDeleted }, // Only update the isDeleted
      { where: { id: req.params.id } } // Match the application by ID
    );
